...
void afficher_inventaire(Jeu &jeu)
{
    ...
    // affichage du titre
    {
        ...
        titre = "INVENTAIRE (T = fermer, ESPACE = utiliser)";
    ...
    if (jeu.nb_inventaire_items > 0)
    {
        ...
        // recherche de la configuration selectionnee
        for (int j = 0; j < jeu.nb_cfg_items; j++)
        {
        ...
        if (cfg_trouvee)
        {
            ecrire_string("details :", 1, y_liste);
            ecrire_string(underscore_espace(cfg_sel.nom), 1, y_liste + 1);
            ecrire_string(
                underscore_espace(cfg_sel.description).substr(0, largeur - 2),
                1,
                y_liste + 2);
            ...
            for (int s = 0; s < NB_STATS; s++)
            {
                ...
                ecrire_string(ligne_stat, 1, y_liste + 4 + s);
            }
        }
    }
}
